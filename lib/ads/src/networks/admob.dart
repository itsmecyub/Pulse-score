...
import 'package:pulse_score/const.dart';
...
class AdmobAD extends Ads {
  ...
  Future<void> loadAppOpenAd() {
    // Completes only once the ad is actually loaded (or failed/timed out), so
    // callers that `await` this — e.g. the splash screen — can guarantee the
    // App Open ad is cached before trying to show it on first launch.
    ...
    AppOpenAd.load(
      ...
      adLoadCallback: AppOpenAdLoadCallback(
        onAdLoaded: (ad) {
          Log.log('Admob >> AppOpenAd loaded');
          _appOpenAd = ad;
          if (!completer.isCompleted) completer.complete();
        },
        onAdFailedToLoad: (error) {
          Log.log('Admob >> AppOpenAd failed to load: $error');
          _appOpenAd = null;
          if (!completer.isCompleted) completer.complete();
        },
      ),
    ...
    // Never let a stalled load block the splash screen indefinitely.
